...
int main() {
     ...
     *
     *     mass:     6.633e-26 kg          = one natural unit of mass for argon,
     *by definition energy:   1.96183e-21 J      = one natural unit of energy
     *for argon, directly from L-J parameters length:   3.3605e-10  m         =
     *one natural unit of length for argon, directly from L-J parameters volume:
     *3.79499-29 m^3        = one natural unit of volume for argon, by length^3
     *     time:     1.951e-12 s           = one natural unit of time for argon,
     *by length*sqrt(mass/energy)
     ***************************************************************************************/
    ...
    if (Vol < N) {
        ...
        printf("  THE NUMBER OF PARTICLES IS %i AND THE AVAILABLE VOLUME IS %f "
               "NATURAL UNITS\n",
               N, Vol);
        printf("  SIMULATIONS WITH DENSITY GREATER THAN 1 PARTCICLE/(1 Natural "
    ...
    L = pow(Vol, (1. / 3)); // Vol^(1/3) <-> raiz cúbica (vol)

    //  Files that we can write different quantities to
    tfp = fopen(tfn, "w"); //  The MD trajectory, coordinates of every particle
                           //  at each timestep
    ofp = fopen(
        ofn,
    ...
    //  velocities that corresponds to the initial temperature we have specified
    initialize();
    ...
    printf("\n  THE FOLLOWING THERMODYNAMIC AVERAGES WILL BE COMPUTED AND "
           "WRITTEN TO THE FILE  \n  '%s':\n",
           afn);
    printf("\n  AVERAGE TEMPERATURE (K):                 %.20f\n", Tavg);
    printf("\n  AVERAGE PRESSURE  (Pa):                  %.20f\n", Pavg);
    printf("\n  PV/nT (J * mol^-1 K^-1):                 %.20f\n", gc);
    printf("\n  PERCENT ERROR of pV/nT AND GAS CONSTANT: %.20f\n",
           100 * fabs(gc - 8.3144598) / 8.3144598);
    printf("\n  THE COMPRESSIBILITY (unitless):          %.20f \n", Z);
...
double Potential() {
    double quot, r2, term1, term2, Pot, rx, ry, rz;
    int i, j, k;

    double *r_sub0 = (double *)aligned_alloc(32, N * sizeof(double));
    double *r_sub1 = (double *)aligned_alloc(32, N * sizeof(double));
    double *r_sub2 = (double *)aligned_alloc(32, N * sizeof(double));
    double *r_2 = (double *)aligned_alloc(32, N * sizeof(double));
    ...
    for (i = 0; i < N; i++) {
        ...
        for (j = i + 1; j < N; j++) {
            r_2[i] = r_sub0[i] + r_sub0[i] + r_sub2[i];
        }
...
void computeAccelerations() {
    ...
    double *r_ij0 = (double *)aligned_alloc(32, N * sizeof(double));
    double *r_ij1 = (double *)aligned_alloc(32, N * sizeof(double));
    double *r_ij2 = (double *)aligned_alloc(32, N * sizeof(double));
    double *r_sqrd = (double *)aligned_alloc(32, N * sizeof(double));
    double *f = (double *)aligned_alloc(32, N * sizeof(double));
    ...
    for (i = 0; i < N - 1; i++) { // loop over all distinct pairs i,j
        ...
        for (j = i + 1; j < N; j++) {
            r_ij0[j] = r_ij0[j] * r_ij0[j];
            r_ij1[j] = r_ij1[j] * r_ij1[j];
            r_ij2[j] = r_ij2[j] * r_ij2[j];
        }
        ...
        for (j = i + 1; j < N; j++) {
            r_sqrd[j] = r_sqrd[j] * r_sqrd[j] * r_sqrd[j];
        }
        // double rSqd_ = rSqd * rSqd * rSqd;
        for (j = i + 1; j < N; j++) {
            f[j] = 24 * (2 - r_sqrd[j]) / (r_sqrd[j] * r_sqrd[j] * r_sqrd[j]);
        }
...
double VelocityVerlet(double dt, int iter, FILE *fp) {
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j < 3; j++) {
            if (r[j][i] < 0. || r[j][i] >= L) {
                v[j][i] = -v[j][i]; //- elastic walls
                psum +=
                    fabs(v[j][i]); // contribution to pressure from "left" walls
            }
        }
    }
