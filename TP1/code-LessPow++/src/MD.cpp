...
double Potential() {
    ...
    double s = (sigma*sigma*sigma)*(sigma*sigma*sigma);
    ...
    for (i = 0; i < N; i++) {
        for (j = i + 1; j < N; j++) {
            ...
            r2 = (rx*rx + ry*ry + rz*rz)*(rx*rx + ry*ry + rz*rz)*(rx*rx + ry*ry + rz*rz);
...
double VelocityVerlet(double dt, int iter, FILE *fp) {
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j < 3; j++) {
            if (r[i][j] < 0. || r[i][j] >= L) {
                v[i][j] = -v[i][j]; //- elastic walls
                psum +=
                    fabs(v[i][j]); // contribution to pressure from "left" walls
            }
        }
    }
