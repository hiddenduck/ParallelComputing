...
double Potential() {
    double quot, r2, term1, term2, Pot;
    int i, j, k;
    ...
    for (i=0; i<N; i++) {
        for (j=0; j<N; j++) {
            ...
            if (j!=i) {
                ...
                quot=sigma/r2; // remoção do rnorm e da sqrt uma vez que múltiplicamos o pow no term1 e no term2 um número par de vezes
                // trocar ordem do term2 e term1 para aproveitar term2 no calculo do term1
                term2 = quot*quot*quot*quot*quot*quot; // pow -> multiplicação
                term1 = term2*quot*quot*quot*quot*quot*quot; // pow -> multiplicação
