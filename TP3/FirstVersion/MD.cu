 ...
 */
 ...
 __global__
 void computeKernel(double *r, double *a, double *pot, double s, int N){ // r -> constant memory, faz uso da constant cache
     int j;
     int i = blockIdx.x*blockDim.x+threadIdx.x;
     if(i<N){
     int lid = threadIdx.x;
 
     __shared__ double sa_x[NUM_THREADS_PER_BLOCK];
     __shared__ double sa_y[NUM_THREADS_PER_BLOCK];
     __shared__ double sa_z[NUM_THREADS_PER_BLOCK];
     __shared__ double spot[NUM_THREADS_PER_BLOCK];
 
     sa_x[lid] = 0;
     sa_y[lid] = 0;
     sa_z[lid] = 0;
     
     double quot, r_sqrd_, r_sqrd, f, rij0, rij1, rij2, aux_x, aux_y, aux_z, tempx, tempy, tempz;
     double Pot = 0;
     ...
     __syncthreads(); // Barreira para evitar data races nos arrays sa_x, sa_y, sa_z
    
     //  From derivative of Lennard-Jones with sigma and epsilon set
     //  equal to 1 in natural units!
     for (j = i + 1; j < N; j++) {
         rij0 = r[i] - r[j];//r[0][i] - r[0][j];
         rij1 = r[N + i] - r[N + j];//r[1][i] - r[1][j];
         rij2 = r[2*N + i] - r[2*N + j];//r[2][i] - r[2][j];
         ...
         __syncthreads(); // isto serializa MUITO o nosso código, vamos ter de trabalhar num workaround disto
     }
     ...
     //a[0][i] = sa_x[lid] + aux_x;
     //a[1][i] = sa_y[lid] + aux_y;
     //a[2][i] = sa_z[lid] + aux_z;
     a[i] = sa_x[lid] + aux_x;
     a[N + i] = sa_y[lid] + aux_y;
     a[2*N + i] = sa_z[lid] + aux_z;
     
     if (lid == 0) pot[blockIdx.x] = spot[0];
    }
 }
 
 __global__
 void reducePot(double *pot_i, double *pot_o, int N){
     int i = blockIdx.x*blockDim.x+threadIdx.x;
     ...
     if (lid == 0) pot_o[blockIdx.x] = spot[0];
     }
 }
 ...
 double computeAccelerations() {
     double *r_dev, *a_dev, *pot, *pot_0;
     int bytes = N * sizeof(double);
 
     cudaMalloc ((void**) &r_dev, 3*bytes);
     cudaMalloc ((void**) &a_dev, 3*bytes);
     cudaMalloc ((void**) &pot, NUM_BLOCKS*sizeof(double));
     checkCUDAError("mem allocation");
     
     cudaMemcpy (r_dev, r, 3*bytes, cudaMemcpyHostToDevice);
     ...
     */
     ...
     startKernelTime ();
     computeKernel <<< NUM_BLOCKS, NUM_THREADS_PER_BLOCK >>> (r_dev, a_dev, pot, sigma*sigma, N);
     stopKernelTime ();
     ...
     cudaMalloc ((void**) &pot_0, sizeof(double));

     startKernelTime ();
     reducePot <<< 1, NUM_THREADS_PER_BLOCK >>> (pot, pot_0, N);
     stopKernelTime ();
     checkCUDAError("kernel invocation");
     
     double POT = 0;
     cudaMemcpy (&POT, pot_0, sizeof(double), cudaMemcpyDeviceToHost);
     checkCUDAError("POT, memcpy d->h");

     /*
     int numberOfBlocks = (NUM_BLOCKS + NUM_THREADS_PER_BLOCK + 1) / NUM_THREADS_PER_BLOCK;
     while(numberOfBlocks > 1){
         reducePot <<< numberOfBlocks, NUM_THREADS_PER_BLOCK >>> ();
         numberOfBlocks = (NUM_BLOCKS + NUM_THREADS_PER_BLOCK + 1) / NUM_THREADS_PER_BLOCK;
     }
     */
 ...
 double VelocityVerlet(double dt, int iter, FILE *fp, double* Pot) {
     ...
     for (i = 0; i < N; i++) {
         for (j = 0; j < 3; j++) {
             r[N*j+i] += dt*(v[j][i] + 0.5 * a[N*j+i] * dt);
 
             v[j][i] += 0.5 * a[N*j+i] * dt;
         }
         // printf("  %i  %6.4e   %6.4e   %6.4e\n",i,r[i][0],r[i][1],r[i][2]);
     }
     //  Update accellerations from updated positions
     *Pot = computeAccelerations();
     ...
     for (i = 0; i < N; i++) {
         for (j = 0; j < 3; j++) {
             if (r[N*j+i] < 0. || r[N*j+i] >= L) {
                 v[j][i] = -v[j][i];    //- elastic walls
                 psum += fabs(v[j][i]); // contribution to pressure from "left" walls
             }
         }
     }
