...
__global__
void computeKernel(double *r, double *a, double *pot, double *a_o){
    int i = blockIdx.y*blockDim.y+threadIdx.y;
    int j = blockIdx.x*blockDim.x+threadIdx.x;
    
    double quot, r_sqrd_, r_sqrd, f, rij0, rij1, rij2, aux[3], tempx, tempy, tempz;
...
double computeAccelerations() {
    ...
    dim3 blocksPerGrid ((N-1)/16, (N-1)/16);
