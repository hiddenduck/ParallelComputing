 ...
 */
 ...
 __global__
 void computeKernel(double *r, double *a, double *pot, double s, double *a_x, double *a_y, double *a_z, int N){
     int j;
 ...
 __global__
 void reducePot(double *pot_i, double *pot_o,  int size, int N){
     int i = blockIdx.x*blockDim.x+threadIdx.x;
     
     if(i < N && i < size){
        
     int lid = threadIdx.x;
     __shared__ double spot[NUM_BLOCKS];
 
     spot[lid] = pot_i[i];

     __syncthreads();
 
     for (unsigned int s=1; s < size && s < blockDim.x; s *= 2) {
        if (lid % (2*s) == 0) {
        spot[lid] += spot[lid + s];
        }
        __syncthreads();
 ...
 double computeAccelerations() {
     ...
     startKernelTime ();
     computeKernel <<< NUM_BLOCKS, NUM_THREADS_PER_BLOCK >>> (r_dev, a_dev, pot, sigma*sigma, ax, ay, az, N);
     stopKernelTime ();
     ...
     startKernelTime ();
     reducePot <<< 1, 512 >>> (pot, pot_0, NUM_BLOCKS, N);
     stopKernelTime ();
     checkCUDAError("reducePot kernel invocation");

     startKernelTime ();
     computeMinus <<< NUM_BLOCKS, NUM_THREADS_PER_BLOCK >>> (ax, ay, az, a, N);
     stopKernelTime ();
