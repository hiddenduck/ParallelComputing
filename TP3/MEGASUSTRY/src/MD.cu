...
__device__ double atomicAdd2(double* address, double val)
{
 ...
 __global__
 void computeKernel(double *r, double *pot, double s, double *a_x, double *a_y, double *a_z, int N, unsigned int *mapping){
     
     int thread_id = blockIdx.x*blockDim.x+threadIdx.x;

     if(thread_id<N-1){
        int i = mapping[2*thread_id];
        int j = mapping[2*thread_id+1];

        double quot, r_sqrd_, r_sqrd, f, rij0, rij1, rij2, tempx, tempy, tempz;
